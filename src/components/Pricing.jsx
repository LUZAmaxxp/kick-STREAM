        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
          {PLANS.map((plan, i) => (
            <motion.div
              ...
              className={`relative bg-accent/[0.08] border-2 border-accent/30 p-8 rounded-[30px] flex flex-col gap-6 transition-all duration-300 hover:border-accent/60 hover:bg-accent/[0.12] ${plan.popular ? 'lg:-translate-y-4' : ''}`}
              style={{ boxShadow: plan.popular ? '0 20px 60px rgba(183, 158, 79, 0.15)' : '0 12px 36px rgba(0,0,0,0.25)' }}
            >
              {/* MOST POPULAR badge */}
              {plan.popular && (
                <div className="absolute -top-4 right-6 bg-accent text-black font-data text-[10px] tracking-[0.2em] uppercase px-3 py-1 font-bold border-2 border-accent/40">
                  MOST POPULAR
                </div>
              )}
              ...
              {/* Plan name */}
              <div>
                <p className="font-data text-[11px] tracking-[0.2em] text-accent font-bold uppercase mb-2">{plan.name}</p>
                <div className="flex items-end gap-1">
                  <span className="font-display text-[56px] leading-none text-accent" style={{ letterSpacing: '-0.02em' }}>
                    {plan.price}
                  </span>
                  <span className="font-ui text-sm text-cream/40 mb-2">{plan.period}</span>
                </div>
